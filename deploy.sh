...
TZC="octez-client --mode mockup --protocol PtKathmankSpLLDALzWw7CGD2j2MtyveTwboEYokqUCP4a1LxMg --base-dir _build/mockup"
...
ligo compile contract ctez.mligo --entry-point main > _build/ctez.tz
ligo compile storage ctez.mligo --entry-point main "$(sed s/DEPLOYMENT_DATE/${DEPLOYMENT_DATE}/ < ctez_initial_storage.mligo)" > _build/ctez_storage.tz
...
ligo compile contract fa12.mligo --entry-point main > _build/fa12.tz
ligo compile storage fa12.mligo --entry-point main "$(sed s/ADMIN_ADDRESS/$CTEZ_ADDRESS/ < fa12_ctez_initial_storage.mligo)" > _build/fa12_ctez_storage.tz
...
ligo compile contract cfmm_tez_ctez.mligo --entry-point main > _build/cfmm.tz
sed s/FA12_CTEZ/${FA12_CTEZ_ADDRESS}/ < cfmm_initial_storage.mligo | sed s/CTEZ_ADDRESS/${CTEZ_ADDRESS}/ > _build/cfmm_storage.mligo

ligo compile storage cfmm_tez_ctez.mligo --entry-point main "$(<_build/cfmm_storage.mligo)" > _build/cfmm_storage.tz
...
ligo compile storage fa12.mligo --entry-point main "$(sed s/ADMIN_ADDRESS/$CFMM_ADDRESS/ < fa12_ctez_initial_storage.mligo)" > _build/fa12_lqt_storage.tz
